      ...
      <section>
        ...
        <div className="grid grid-cols-3 md:grid-cols-7 gap-3 md:gap-5 m-2 justify-items-center">
          {Array.from({ length: 7 }).map((_, i) => (
            <div
              key={i}
              className="bg-[#3a3a5c] h-40 w-28 md:w-25 rounded-2xl border-gray-500 border-x mb-2 flex flex-col items-center justify-between p-3"
            >
            ...
            <div className="grid grid-cols-3 md:grid-cols-7 gap-3 md:gap-5 m-2 justify-items-center">
                {
                    (days.slice(0, 7).map((dayTime, i) => {
                        ...
                        const max = maxRaw !== undefined && units ? roundTemp(toTemp(maxRaw, units.temp)) : (maxRaw ?? "-")
                         ...
                         return (
                            <div key={dayTime} className="bg-[#3a3a5c] h-40 w-28 md:w-25 rounded-2xl border-gray-500 border-x mb-2 flex flex-col items-center justify-between p-3">
                                <h4 className="text-gray-300 font-dm-sans text-2xl text-center">{label}</h4>
                                {icon && <img src={icon} alt="icon" className="h-12 w-12" />}
                                <div className="w-full flex justify-between text-gray-200 font-dm-sans text-lg">
